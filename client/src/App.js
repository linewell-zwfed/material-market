        ...
        <Form.Item label="选择物料：">
          <Collapse>
            <Panel header="列表页" key="1">
              <Radio.Group
                buttonStyle="solid"
                onChange={(e) => onTempChange(temps.list, e.target.value)}
                value={tempKey}
              >
                {Object.entries(temps.list).map(([key, value]) => (
            ...
            <Panel header="详情页" key="2">
              <Radio.Group
                buttonStyle="solid"
                onChange={(e) => onTempChange(temps.details, e.target.value)}
                value={tempKey}
              >
                {Object.entries(temps.details).map(([key, value]) => (
                  <Radio.Button value={key} key={key}>
                    {value.label}
                  </Radio.Button>
                ))}
            ...
            <Panel header="操作项" key="3">
              <Radio.Group
                buttonStyle="solid"
                onChange={(e) => onTempChange(temps.operations, e.target.value)}
                value={tempKey}
              >
                {Object.entries(temps.operations).map(([key, value]) => (
                  <Radio.Button value={key} key={key}>
                    {value.label}
                  </Radio.Button>
                ))}
            ...
            <Panel header="表单" key="4">
              <Radio.Group
                buttonStyle="solid"
                onChange={(e) => onTempChange(temps.forms, e.target.value)}
                value={tempKey}
              >
                {Object.entries(temps.forms).map(([key, value]) => (
                  <Radio.Button value={key} key={key}>
                    {value.label}
                  </Radio.Button>
                ))}
